...
/// [`UpdateMode`].
struct WinitAppRunnerState<T: GlobalEvent> {
    /// The running app.
    app: App,
...
impl<T: GlobalEvent> WinitAppRunnerState<T> {
...
impl<T: GlobalEvent> ApplicationHandler<T> for WinitAppRunnerState<T> {
...
/// `EventLoop`.
pub fn winit_runner<T: GlobalEvent>(mut app: App, event_loop: EventLoop<T>) -> AppExit {
