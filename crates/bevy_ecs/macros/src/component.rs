...
pub const TARGETED_EVENT: &str = "targeted_event";
...
pub fn derive_global_event(input: TokenStream) -> TokenStream {
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics #bevy_ecs_path::event::GlobalEvent for #struct_name #type_generics #where_clause {}
    })
}
...
pub fn derive_targeted_event(input: TokenStream) -> TokenStream {
    ...
    if let Some(attr) = ast
        .attrs
        .iter()
        .find(|attr| attr.path().is_ident(TARGETED_EVENT))
    {
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics #bevy_ecs_path::event::TargetedEvent for #struct_name #type_generics #where_clause {
