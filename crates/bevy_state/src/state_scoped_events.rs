...
use bevy_ecs::{
    event::{EventReader, Events, GlobalEvent},
    resource::Resource,
    system::Commands,
    world::World,
};
...
fn clear_event_queue<E: GlobalEvent>(w: &mut World) {
...
impl<S: States> StateScopedEvents<S> {
    fn add_event<E: GlobalEvent>(&mut self, state: S, transition_type: TransitionType) {
...
fn clear_events_on_state_transition<E: GlobalEvent, S: States>(
    app: &mut SubApp,
    _p: PhantomData<E>,
    state: S,
...
pub trait StateScopedEventsAppExt {
    /// Clears an [`GlobalEvent`] when exiting the specified `state`.
    ///
    ...
    fn clear_events_on_exit_state<E: GlobalEvent>(&mut self, state: impl States) -> &mut Self;

    /// Clears an [`GlobalEvent`] when entering the specified `state`.
    ///
    /// Note that event cleanup is ambiguously ordered relative to
    ...
    fn clear_events_on_enter_state<E: GlobalEvent>(&mut self, state: impl States) -> &mut Self;
}
